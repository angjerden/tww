...
void daSTBox_c::CreateInit() {
    ...
    position.x = craneTop.x;
    position.y = craneTop.y;
    position.z = craneTop.z;
    ...
    if (field_0x331 == 1 || field_0x331 == 2) {
        ...
        JPABaseEmitter* emitter = dComIfGp_particle_setShipTail(dPa_name::ID_AK_JN_SALVAGE00, &position, 
                &current.angle, NULL, 0, &shadowCallback, -1, 
                NULL, NULL, NULL);
            shadowCallback.setMPos(position);
